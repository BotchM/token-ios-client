...
#import "PhotoAvatarCropView.h"
#import "ModernButton.h"

#import "PhotoPaintController.h"
#import "Common.h"

const CGFloat PhotoAvatarCropButtonsWrapperSize = 61.0f;

@interface PhotoAvatarCropController ()
{
    UIView *_wrapperView;
...
@implementation PhotoAvatarCropController
...
- (void)loadView
{
    ...
    __weak PhotoAvatarCropController *weakSelf = self;
    void(^interactionEnded)(void) = ^
    {
        __strong PhotoAvatarCropController *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
    ...
    _cropView = [[PhotoAvatarCropView alloc] initWithOriginalSize:photoEditor.originalSize screenSize:[self referenceViewSize]];
...
- (void)transitionOutSwitching:(bool)switching completion:(void (^)(void))completion
{
    ...
    if (switching)
    {
        ...
        CGRect containerFrame = [PhotoEditorTabController photoContainerFrameForParentViewFrame:referenceBounds toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:PhotoEditorPanelSize];
        
        if (self.switchingToTab == PhotoEditorPaintTab)
        {
            containerFrame = [PhotoPaintController photoContainerFrameForParentViewFrame:referenceBounds toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:PhotoPaintTopPanelSize + PhotoPaintBottomPanelSize];
        }
        
        CGSize fittedSize = ScaleToSize(cropRectFrame.size, containerFrame.size);
...
- (void)transitionOutSaving:(bool)__unused saving completion:(void (^)(void))completion
{
    ...
    POPSpringAnimation *alphaAnimation = [PhotoEditorAnimation prepareTransitionAnimationForPropertyNamed:kPOPViewAlpha];
...
- (CGRect)_targetFrameForTransitionInFromFrame:(CGRect)fromFrame
{
    ...
    CGRect containerFrame = [PhotoEditorTabController photoContainerFrameForParentViewFrame:CGRectMake(0, 0, referenceSize.width, referenceSize.height) toolbarLandscapeSize:self.toolbarLandscapeSize orientation:orientation panelSize:0.0f];

    CGRect targetFrame = CGRectZero;
    
    CGFloat shortSide = MIN(referenceSize.width, referenceSize.height);
    CGFloat diameter = shortSide - [PhotoAvatarCropView areaInsetSize].width * 2;
    if (self.initialAppearance && (self.fromCamera || !self.skipTransitionIn))
    {
        ...
        CGSize fittedSize = ScaleToFill(referenceSize, CGSizeMake(diameter, diameter));
...
- (void)updateLayout:(UIInterfaceOrientation)orientation
{
    ...
    switch (orientation)
    {
        case UIInterfaceOrientationLandscapeLeft:
        {
            [UIView performWithoutAnimation:^
            {
                _buttonsWrapperView.frame = CGRectMake(screenEdges.left + self.toolbarLandscapeSize,
                                                       screenEdges.top,
                                                       PhotoAvatarCropButtonsWrapperSize,
        ...
        case UIInterfaceOrientationLandscapeRight:
        {
            [UIView performWithoutAnimation:^
            {
                _buttonsWrapperView.frame = CGRectMake(screenEdges.right - self.toolbarLandscapeSize - PhotoAvatarCropButtonsWrapperSize,
                                                       screenEdges.top,
                                                       PhotoAvatarCropButtonsWrapperSize,
                                                       referenceSize.height);
        ...
        default:
        {
            [UIView performWithoutAnimation:^
            {
                _buttonsWrapperView.frame = CGRectMake(screenEdges.left,
                                                       screenEdges.bottom - PhotoEditorToolbarSize - PhotoAvatarCropButtonsWrapperSize,
                                                       referenceSize.width,
                                                       PhotoAvatarCropButtonsWrapperSize);
