...
const CGFloat MenuSheetPadMenuWidth = 375.0f;
const CGFloat MenuSheetDefaultStatusBarHeight = 20.0f;

typedef enum
{
    MenuSheetAnimationChange,
    MenuSheetAnimationDismiss,
    MenuSheetAnimationPresent,
    MenuSheetAnimationFastDismiss
} MenuSheetAnimation;

typedef enum
{
    MenuPanDirectionHorizontal,
    MenuPanDirectionVertical,
} MenuPanDirection;

@interface MenuPanGestureRecognizer : UIPanGestureRecognizer

@property (nonatomic, assign) MenuPanDirection direction;

@end

@interface MenuSheetController () <UIGestureRecognizerDelegate, UIPopoverPresentationControllerDelegate, UIPopoverControllerDelegate, UIViewControllerPreviewingDelegate>
{
    UIView *_containerView;
    MenuSheetDimView *_dimView;
    MenuSheetView *_sheetView;
    bool _presented;
    ...
    MenuPanGestureRecognizer *_gestureRecognizer;
...
@implementation MenuSheetController
...
- (void)loadView
{
    ...
    if (CurrentSizeClass() == UIUserInterfaceSizeClassCompact)
    {
...
//    {
//        __strong MenuSheetController *strongSelf = weakSelf;
//        if (strongSelf == nil)
//            return;
//        
...
//    }]];
    ...
    _dimView = [[MenuSheetDimView alloc] initWithActionMenuView:_sheetView];
...
- (void)setItemViews:(NSArray *)itemViews animated:(bool)animated
{
    ...
    void (^menuRelayout)(void) = ^
    {
        ...
        [strongSelf repositionMenuWithReferenceSize:[UIScreen mainScreen].bounds.size];
    };
    
    if (animated && compact)
    {
        MenuSheetView *sheetView = _sheetView;
        ...
        [self animateSheetViewToPosition:0 velocity:0 type:MenuSheetAnimationPresent completion:^
        {
...
- (UIUserInterfaceSizeClass)sizeClass
{
    ...
    return UIUserInterfaceSizeClassCompact;//sizeClass;
}

- (bool)isInPopover
{
//    if ([_parentController isKindOfClass:[UINavigationController class]])
//    {
//        UINavigationController *navController = (UINavigationController *)_parentController;
...
- (void)presentInViewController:(UIViewController *)viewController sourceView:(UIView *)sourceView animated:(bool)animated
{
    ...
    if ([_parentController.presentedViewController isKindOfClass:[MenuSheetController class]])
        return;
    
    for (UIViewController *controller in _parentController.childViewControllers)
    {
        if ([controller isKindOfClass:[MenuSheetController class]])
            return;
    }
...
//            ((TGNavigationController *)self.parentViewController).interactivePopGestureRecognizer.enabled = true;
        ...
        if (animated)
        {
            ...
            [self animateSheetViewToPosition:_sheetView.menuHeight velocity:0 type:MenuSheetAnimationDismiss completion:^
            {
    ...
    else
    {
        ...
        void (^dismissedBlock)(void) = ^
        {
            ...
            if ([self.parentViewController isKindOfClass:[OverlayController class]]) {
                OverlayControllerWindow *window = ((OverlayController *)self.parentViewController).overlayWindow;
...
- (void)animateSheetViewToPosition:(CGFloat)position velocity:(CGFloat)velocity type:(MenuSheetAnimation)type completion:(void (^)(void))completion
{
...
- (void)updateGestureRecognizer
{
    ...
    if (_hasSwipeGesture && [self sizeClass] != UIUserInterfaceSizeClassRegular)
    {
        ...
        _gestureRecognizer = [[MenuPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
        _gestureRecognizer.direction = MenuPanDirectionVertical;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
    ...
    {
        if ([otherGestureRecognizer.view isKindOfClass:[MenuSheetCollectionView class]])
        {
            MenuSheetCollectionView *collectionView = (MenuSheetCollectionView *)otherGestureRecognizer.view;
            return collectionView.allowSimultaneousPan;
        }
        else if ([otherGestureRecognizer.view isKindOfClass:[MenuSheetScrollView class]])
        {
            return true;
        }
    ...
    switch (gestureRecognizer.state)
    {
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if (velocity > 200.0f && allowDismissal)
            {
                ...
                [self animateSheetViewToPosition:_sheetView.menuHeight velocity:velocity type:MenuSheetAnimationDismiss completion:^
                {
            ...
            {
                [self animateSheetViewToPosition:0 velocity:0 type:MenuSheetAnimationChange completion:nil];
            }
        }
            break;
...
- (void)repositionMenuWithReferenceSize:(CGSize)referenceSize
{
    ...
    referenceSize.height = referenceSize.height + MenuSheetDefaultStatusBarHeight - [self statusBarHeight];
...
- (CGFloat)statusBarHeight
{
    ...
    statusBarHeight = MAX(MenuSheetDefaultStatusBarHeight, statusBarHeight);
...
- (void)updateKeyboardOffset:(CGFloat)keyboardOffset
{
    ...
    [self repositionMenuWithReferenceSize:[UIScreen mainScreen].bounds.size];
    [_sheetView layoutSubviews];
}
...
- (void)popoverPresentationControllerDidDismissPopover:(UIPopoverPresentationController *)__unused popoverPresentationController {
    ...
    } else if ([self.parentController isKindOfClass:[OverlayController class]]) {
        OverlayControllerWindow *window = ((OverlayController *)self.parentController).overlayWindow;
        if (window.dismissByMenuSheet) {
            [window dismiss];
        }
    }
...
@implementation MenuPanGestureRecognizer
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    {
        case MenuPanDirectionHorizontal:
            if (fabs(velocity.y) > fabs(velocity.x))
                self.state = UIGestureRecognizerStateCancelled;
            break;
            
        case MenuPanDirectionVertical:
