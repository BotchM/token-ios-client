...
@interface AttachmentFileTipView ()
{
    ModernButton *_closeButton;
...
@implementation AttachmentFileTipView

- (instancetype)init
{
    self = [super initWithType:MenuSheetItemTypeHeader];
    if (self != nil)
    {
        _imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"AttachmentTipIcons"]];
        [self addSubview:_imageView];
        
        NSString *text = [[NSString alloc] initWithFormat:TGLocalized(@"Conversation.FileHowToText"), [StringUtils stringForDeviceType]];
        ...
        _closeButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
...
{
    bool isCompact = CurrentSizeClass() == UIUserInterfaceSizeClassCompact;
