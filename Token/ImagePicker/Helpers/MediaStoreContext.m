...
@interface MediaStoreContext ()
{
    ...
    MemoryImageCache *_mediaImageCache;
    MemoryImageCache *_mediaReducedImageCache;
    ...
    ModernCache *_temporaryFilesCache;
}

@end

@implementation MediaStoreContext
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        _mediaImageCache = [[MemoryImageCache alloc] initWithSoftMemoryLimit:(NSUInteger)(2 * 1024 * 1024 * factor) hardMemoryLimit:(NSUInteger)(3 * 1024 * 1024 * factor)];
        _mediaReducedImageCache = [[MemoryImageCache alloc] initWithSoftMemoryLimit:(NSUInteger)(1.5 * 1024 * 1024 * factor) hardMemoryLimit:(NSUInteger)(2.5 * 1024 * 1024 * factor)];
...
//        _temporaryFilesCache = [[ModernCache alloc] initWithPath:tempCachePath size:100 * 1024 * 1024];
...
{
    static MediaStoreContext *singleton = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^
    {
        singleton = [[MediaStoreContext alloc] init];
    });
    return singleton;
}
