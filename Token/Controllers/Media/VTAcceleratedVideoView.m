...
@interface VTAcceleratedVideoFrame : NSObject
...
@implementation VTAcceleratedVideoFrame
...
@class VTAcceleratedVideoFrameQueue;
@class VTAcceleratedVideoFrameQueueGuard;

@interface VTAcceleratedVideoFrameQueueItem : NSObject

@property (nonatomic, strong) VTAcceleratedVideoFrameQueue *queue;
@property (nonatomic, strong) NSMutableArray *guards;

@end

@implementation VTAcceleratedVideoFrameQueueItem
...
@interface VTAcceleratedVideoFrameQueueGuardItem : NSObject

@property (nonatomic, weak) VTAcceleratedVideoFrameQueueGuard *guard;
@property (nonatomic, strong) NSObject *key;

@end

@implementation VTAcceleratedVideoFrameQueueGuardItem

- (instancetype)initWithGuard:(VTAcceleratedVideoFrameQueueGuard *)guard key:(NSObject *)key {
    self = [super init];
    if (self != nil) {
        self.guard = guard;
        self.key = key;
...
@interface VTAcceleratedVideoFrameQueueGuard : NSObject {
    void (^_draw)(VTAcceleratedVideoFrame *);
    NSString *_path;
}

@property (nonatomic, strong) NSObject *key;

- (void)draw:(VTAcceleratedVideoFrame *)frame;

- (instancetype)initWithDraw:(void (^)(VTAcceleratedVideoFrame *))draw path:(NSString *)path;

@end

@interface VTAcceleratedVideoFrameQueue : NSObject
{
    int32_t _sessionId;
    SQueue *_queue;
    void (^_frameReady)(VTAcceleratedVideoFrame *);
...
@implementation VTAcceleratedVideoFrameQueue
...
- (void)checkQueue {
    ...
    if (_frames.count != 0) {
        ...
        VTAcceleratedVideoFrame *firstFrame = _frames[0];
    ...
    if (_frames.count <= _fillFrames) {
        while (_frames.count < _maxFrames) {
            VTAcceleratedVideoFrame *frame = [self requestFrame];
    ...
    __weak VTAcceleratedVideoFrameQueue *weakSelf = self;
    _timer = [[STimer alloc] initWithTimeout:nextDelay repeat:false completion:^{
        __strong VTAcceleratedVideoFrameQueue *strongSelf = weakSelf;
        if (strongSelf != nil) {
...
static void VTPlayerDecompressionOutputCallback(void *decompressionOutputRefCon, __unused void *sourceFrameRefCon, OSStatus status, __unused VTDecodeInfoFlags infoFlags, CVImageBufferRef imageBuffer, CMTime presentationTimestamp, __unused CMTime presentationDuration) {
    @autoreleasepool {
        if (status != noErr) {
            ...
            TGLog(@"VTPlayerDecompressionOutputCallback error %d", (int)status);
            return;
        }
        ...
        if (CMTIME_IS_INVALID(presentationTimestamp)) {
            TGLog(@"VTPlayerDecompressionOutputCallback invalid timestamp");
            return;
        }
        ...
        VTAcceleratedVideoFrameQueue *queue = sessionReference.object;
        VTAcceleratedVideoFrame *frame = [[VTAcceleratedVideoFrame alloc] initWithBuffer:imageBuffer timestamp:presentationSeconds angle:queue.angle];
...
- (VTAcceleratedVideoFrame *)requestFrame {
    _failed = false;
    for (int i = 0; i < 3; i++) {
        if (_reader == nil) {
        ...
        if (_reader != nil) {
            ...
            if (([_reader status] == AVAssetReaderStatusReading) && (sampleVideo = [_output copyNextSampleBuffer])) {
                VTAcceleratedVideoFrame *videoFrame = nil;
                if (_useVT) {
                    if (_decompressionSession != NULL) {
                    ...
                    if (_pendingFrames.count >= 3) {
                        VTAcceleratedVideoFrame *earliestFrame = nil;
                        for (VTAcceleratedVideoFrame *frame in _pendingFrames) {
                    ...
                    videoFrame = [[VTAcceleratedVideoFrame alloc] initWithBuffer:imageBuffer timestamp:presentationSeconds angle:_angle];
                }
...
@implementation VTAcceleratedVideoFrameQueueGuard
...
+ (void)addGuardForPath:(NSString *)path guard:(VTAcceleratedVideoFrameQueueGuard *)guard {
    ...
    VTAcceleratedVideoFrameQueueItem *item = queueItemsByPath()[path];
    if (item == nil) {
        item = [[VTAcceleratedVideoFrameQueueItem alloc] init];
        __weak VTAcceleratedVideoFrameQueueItem *weakItem = item;
        item.queue = [[VTAcceleratedVideoFrameQueue alloc] initWithPath:path frameReady:^(VTAcceleratedVideoFrame *frame) {
            [[self controlQueue] dispatch:^{
                __strong VTAcceleratedVideoFrameQueueItem *strongItem = weakItem;
                if (strongItem != nil) {
                    for (VTAcceleratedVideoFrameQueueGuardItem *guardItem in strongItem.guards) {
    ...
    [item.guards addObject:[[VTAcceleratedVideoFrameQueueGuardItem alloc] initWithGuard:guard key:guard.key]];
}
    ...
    [[self controlQueue] dispatch:^{
        ...
        if (item != nil) {
            for (NSInteger i = 0; i < (NSInteger)item.guards.count; i++) {
                VTAcceleratedVideoFrameQueueGuardItem *guardItem = item.guards[i];
...
- (void)dealloc {
    [VTAcceleratedVideoFrameQueueGuard removeGuardFromPath:_path key:_key];
}
...
@interface VTAcceleratedVideoContext : NSObject {
    SQueue *_queue;
...
@implementation VTAcceleratedVideoContext

+ (VTAcceleratedVideoContext *)instance {
    static VTAcceleratedVideoContext *singleton = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        singleton = [[VTAcceleratedVideoContext alloc] init];
    });
    return singleton;
}
...
@interface VTAcceleratedVideoView () {
    CAEAGLLayer *_layer;
    ...
    VTAcceleratedVideoFrameQueueGuard *_frameQueueGuard;
...
@implementation VTAcceleratedVideoView
+ (Class)layerClass {
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self != nil) {
        //TGLog(@"VTAcceleratedVideoView count %d", OSAtomicIncrement32(&VTAcceleratedVideoViewCount));
        
        _applicationWillEnterForeground = [[ObserverProxy alloc] initWithTarget:self targetSelector:@selector(applicationDidBecomeActiveNotification:) name:UIApplicationDidBecomeActiveNotification];
        _applicationDidEnterBackground = [[ObserverProxy alloc] initWithTarget:self targetSelector:@selector(applicationWillResignActiveNotification:) name:UIApplicationWillResignActiveNotification];
        _applicationDidEnterBackground2 = [[ObserverProxy alloc] initWithTarget:self targetSelector:@selector(applicationWillResignActiveNotification:) name:UIApplicationDidEnterBackgroundNotification];
        ...
        [[VTAcceleratedVideoContext instance] performAsynchronouslyWithContext:^{
...
- (void)dealloc {
    //TGLog(@"VTAcceleratedVideoView count %d", OSAtomicDecrement32(&VTAcceleratedVideoViewCount));
...
- (void)setupBuffers {
    ...
    [[VTAcceleratedVideoContext instance] renderbufferStorage:GL_RENDERBUFFER fromDrawable:_layer];
...
- (void)displayFrame:(VTAcceleratedVideoFrame *)frame
{
    ...
    if (!_inBackground && _buffersInitialized) {
        ...
        if (frame.buffer != NULL) {
            ...
            if ([VTAcceleratedVideoContext instance].videoTextureCache == NULL) {
                ...
                CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [VTAcceleratedVideoContext instance].videoTextureCache, frame.buffer, NULL, GL_TEXTURE_2D, GL_RED_EXT, frameWidth, frameHeight, GL_RED_EXT, GL_UNSIGNED_BYTE, 0, &lumaTexture);
                if (lumaTexture == NULL) {
                ...
                CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, [VTAcceleratedVideoContext instance].videoTextureCache, frame.buffer, NULL, GL_TEXTURE_2D, GL_RG_EXT, frameWidth / 2, frameHeight / 2, GL_RG_EXT, GL_UNSIGNED_BYTE, 1, &chromaTexture);
                ...
                glUseProgram([VTAcceleratedVideoContext instance].program);
                glUniform1f([VTAcceleratedVideoContext instance].uniforms[UniformIndex_RotationAngle], 0.0f);
                glUniformMatrix3fv([VTAcceleratedVideoContext instance].uniforms[UniformIndex_ColorConversionMatrix], 1, false, _preferredConversion);
                ...
                CGSize renderSize = ScaleToFill(imageSize, boundsSize);
        ...
        [[VTAcceleratedVideoContext instance] presentRenderbuffer:GL_RENDERBUFFER];
...
- (void)setPath:(NSString *)path {
    [[VTAcceleratedVideoFrameQueueGuard controlQueue] dispatch:^{
        ...
        if (!StringCompare(realPath, _path)) {
            _path = realPath;
            _frameQueueGuard = nil;
            
            if (_path.length != 0) {
                __weak VTAcceleratedVideoView *weakSelf = self;
                _frameQueueGuard = [[VTAcceleratedVideoFrameQueueGuard alloc] initWithDraw:^(VTAcceleratedVideoFrame *frame) {
                    __strong VTAcceleratedVideoView *strongSelf = weakSelf;
                    if (strongSelf != nil) {
                        ...
                        if (scheduleDisplay) {
                            [[VTAcceleratedVideoContext instance] performAsynchronouslyWithContext:^{
                                __strong VTAcceleratedVideoView *strongSelf = weakSelf;
                                if (strongSelf != nil) {
                                    VTAcceleratedVideoFrame *pendingFrame = nil;
                                    OSSpinLockLock(&strongSelf->_pendingFramesLock);
                ...
                [VTAcceleratedVideoFrameQueueGuard addGuardForPath:_path guard:_frameQueueGuard];
            }
        }
