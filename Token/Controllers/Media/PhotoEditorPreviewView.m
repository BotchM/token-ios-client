...
@interface PhotoEditorPreviewView ()
{
...
@implementation PhotoEditorPreviewView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        _imageView = [[PhotoEditorView alloc] initWithFrame:self.bounds];
...
- (void)setPaintingImageWithData:(PaintingData *)data
{
    if (data == nil)
    {
    ...
    switch (gestureRecognizer.state)
    {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (self.touchedDown != nil)
            {
                ...
                [self setOriginalLabelText:TGLocalized(@"Current")];
            }
            else
            {
                [self setActualImageHidden:true animated:false];
                
                [self setOriginalLabelText:TGLocalized(@"Original")];
            }
...
- (void)layoutSubviews
{
    ...
    _paintingView.center = PaintAddPoints(PaintCenterOfRect(_paintingContainerView.bounds), centerOffset);
}
