...
@implementation VideoMediaAttachment
...
- (id)init
{
    ...
    if (self != nil)
    {
        self.type = VideoMediaAttachmentType;
    }
    return self;
}
...
{
    VideoMediaAttachment *videoAttachment = [[VideoMediaAttachment alloc] init];
...
{
    if (![object isKindOfClass:[VideoMediaAttachment class]])
        return false;
    
    VideoMediaAttachment *other = object;
...
- (MediaAttachment *)parseMediaAttachment:(NSInputStream *)is
{
    ...
    if (hasVideoInfo != 0)
        videoAttachment.videoInfo = [VideoInfo deserialize:is];
    
    uint8_t hasThumbnailInfo = 0;
    [is read:&hasThumbnailInfo maxLength:1];
    
    if (hasThumbnailInfo != 0)
        videoAttachment.thumbnailInfo = [ImageInfo deserialize:is];
...
- (NSArray *)textCheckingResults
{
    ...
    {
//        NSArray *textCheckingResults = [Message textCheckingResultsForText:_caption highlightMentionsAndTags:true highlightCommands:true entities:nil];
