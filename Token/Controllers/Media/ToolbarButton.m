...
static UIColor *textColorForButton(int type)
{
    switch (type)
    {
        ...
        case ToolbarButtonTypeDoneBlack:
        {
            break;
        }
...
- (id)initWithCustomImages:(UIImage *)imageNormal imageNormalHighlighted:(UIImage *)imageNormalHighlighted imageLandscape:(UIImage *)imageLandscape imageLandscapeHighlighted:(UIImage *)imageLandscapeHighlighted textColor:(UIColor *)textColor shadowColor:(UIColor *)shadowColor
{
    ...
    if (self != nil)
    {
        ...
        _buttonLabelView = [[UILabel alloc] init];
        _buttonLabelView.font = [UIFont boldSystemFontOfSize:12];
        _buttonLabelView.textColor = _customTextColor != nil ? _customTextColor : textColorForButton(ToolbarButtonTypeGeneric);
        _buttonLabelView.shadowColor = _customShadowColor != nil ? _customShadowColor : shadowColorForButton(ToolbarButtonTypeGeneric);
        _buttonLabelView.shadowOffset = CGSizeMake(0, -1);
        _buttonLabelView.backgroundColor = [UIColor clearColor];
        [self addSubview:_buttonLabelView];
...
- (bool)backSemantics
{
    return _type == ToolbarButtonTypeBack || _backSemantics;
}
...
- (void)updateBackground
{
    ...
    if (_type == ToolbarButtonTypeCustom)
    {
    ...
    else if (_type == ToolbarButtonTypeGeneric)
    {
    ...
    else if (_type == ToolbarButtonTypeBack)
    {
    ...
    else if (_type == ToolbarButtonTypeDone)
    {
    ...
    else if (_type == ToolbarButtonTypeDoneBlack)
    {
    ...
    else if (_type == ToolbarButtonTypeImage)
    {
        background = nil;
    }
    else if (_type == ToolbarButtonTypeDelete)
    {
...
- (void)layoutSubviews
{
    ...
    if (!_buttonLabelView.hidden)
    {
        ...
        labelFrame.origin.y = (float)((bounds.size.height - labelFrame.size.height) / 2) - (!_isLandscape ? 0 : 1.0f) - retinaPixel + addY - (_type != ToolbarButtonTypeBack && !_isLandscape ? retinaPixel : 0.0f);
    }
    else
    ...
    if (_type == ToolbarButtonTypeBack && _isLandscape)
...
- (CGRect)backgroundRectForBounds:(CGRect)bounds
{
    ...
    if (_type == ToolbarButtonTypeBack)
    {
