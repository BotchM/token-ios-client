#import "ModernMediaListController.h"
...
@interface ModernMediaListController () <UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    ...
    ModernMediaListLayout *_collectionLayout;
    UIView *_collectionContainer;
    NSMutableDictionary *_reusableItemContentViewsByIdentifier;
    
    id<ModernMediaListItem> _hiddenItem;
    
    void (^_recycleItemContentView)(ModernMediaListItemContentView *);
    NSMutableArray *_storedItemContentViews;
}

@end

@implementation ModernMediaListController
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        __weak ModernMediaListController *weakSelf = self;
        _recycleItemContentView = ^(ModernMediaListItemContentView *itemContentView)
        {
            __strong ModernMediaListController *strongSelf = weakSelf;
...
- (void)setModel:(ModernMediaListModel *)model
{
    if (_model != model)
    {
        _model = model;
        ...
        _model.itemUpdated = ^(id<ModernMediaListItem> item)
        {
            __strong ModernMediaListController *strongSelf = weakSelf;
            if (strongSelf != nil)
            {
                for (ModernMediaListItemView *itemView in strongSelf->_collectionView.visibleCells)
                {
...
- (void)loadView
{
    ...
    _collectionLayout = [[ModernMediaListLayout alloc] init];
    ...
    [_collectionView registerClass:[ModernMediaListItemView class] forCellWithReuseIdentifier:@"ModernMediaListItemView"];
...
- (void)reloadData
{
    for (ModernMediaListItemView *itemView in [_collectionView visibleCells])
    {
        ModernMediaListItemContentView *itemContentView = [itemView _takeItemContentView];
    ...
    for (ModernMediaListItemContentView *itemContentView in _storedItemContentViews)
    {
...
- (ModernMediaListItemContentView *)dequeueViewForItem:(id<ModernMediaListItem>)item
{
    ...
    if (views.count == 0)
    {
        ...
        ModernMediaListItemContentView *itemView = [[itemClass alloc] init];
        [itemView setItem:item];
        
        return itemView;
    }
    
    ModernMediaListItemContentView *itemView = [views lastObject];
...
- (void)enqueueView:(ModernMediaListItemContentView *)itemView
{
    if (itemView == nil)
        return;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (index < 0)
    {
        ModernMediaListItemView *itemView = (ModernMediaListItemView *)[collectionView dequeueReusableCellWithReuseIdentifier:@"ModernMediaListItemView" forIndexPath:indexPath];
        return itemView;
    }
    else
    {
        id<ModernMediaListItem> item = _model.items[index];
        ...
        ModernMediaListItemContentView *itemContentView = nil;
        if (_storedItemContentViews.count != 0)
        {
            NSInteger index = -1;
            for (ModernMediaListItemContentView *stroredItemContentView in _storedItemContentViews)
            {
                index++;
...
- (void)collectionView:(UICollectionView *)__unused collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        __weak ModernMediaListController *weakSelf = self;
        ModernGalleryController *controller = [_model createGalleryControllerForItem:_model.items[index] hideItem:^(id<ModernMediaListItem> item)
        {
            __strong ModernMediaListController *strongSelf = weakSelf;
            if (strongSelf != nil)
            {
        ...
        } referenceViewForItem:^UIView *(id<ModernMediaListItem> item)
        {
            if (item == nil)
                return nil;
        ...
        if (controller != nil)
        {
            OverlayControllerWindow *controllerWindow = [[OverlayControllerWindow alloc] initWithParentController:self contentController:controller];
