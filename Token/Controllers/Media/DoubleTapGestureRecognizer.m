...
@interface DoubleTapGestureRecognizer () {
...
@implementation DoubleTapGestureRecognizer
...
- (void)failGesture
{
    ...
    if ([self.delegate respondsToSelector:@selector(gestureRecognizerDidFail:)])
        [(id<DoubleTapGestureRecognizerDelegate>)self.delegate gestureRecognizerDidFail:self];
}
...
- (void)touchesMoved:(NSSet *)__unused touches withEvent:(UIEvent *)__unused event
{
    ...
    if (distance.x * distance.x + distance.y * distance.y > 4.0 * 4.0) {
        ...
        {
            if (![(id<DoubleTapGestureRecognizerDelegate>)self.delegate gestureRecognizerShouldFailOnMove:self])
                return;
        }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    if (_currentTapCount == 1)
    {
        ...
        int failTapType = 0;
        if ([self.delegate conformsToProtocol:@protocol(DoubleTapGestureRecognizerDelegate)] && (failTapType = [(id<DoubleTapGestureRecognizerDelegate>)self.delegate gestureRecognizer:self shouldFailTap:[touch locationInView:self.view]]))
        {
            _doubleTapped = false;
...
- (void)tapTimerEvent
{
    ...
    if ([self.delegate conformsToProtocol:@protocol(DoubleTapGestureRecognizerDelegate)] && [self.delegate respondsToSelector:@selector(doubleTapGestureRecognizerSingleTapped:)])
    {
        [(id<DoubleTapGestureRecognizerDelegate>)self.delegate doubleTapGestureRecognizerSingleTapped:self];
    }
...
- (void)longTapEvent
{
    ...
    {
        if ([(id<DoubleTapGestureRecognizerDelegate>)self.delegate gestureRecognizerShouldHandleLongTap:self])
        {
            _longTapped = true;
...
- (bool)canScrollViewStealTouches
{
    ...
    {
        if ([(id<DoubleTapGestureRecognizerDelegate>)self.delegate gestureRecognizerShouldLetScrollViewStealTouches:self])
            return true;
        else
            return false;
    }
