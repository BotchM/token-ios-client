...
const CGFloat CameraModeControlVerticalInteritemSpace = 29.0f;

@interface CameraModeControl ()
{
...
@implementation CameraModeControl
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        dispatch_once(&onceToken, ^
        {
            ...
            CGContextSetFillColorWithColor(context, [CameraInterfaceAssets accentColor].CGColor);
        ...
        if (frame.size.width > frame.size.height)
        {
            ...
            for (UIButton *button in _buttons)
            {
                ...
                leftOffset += button.frame.size.width + [CameraModeControl _buttonHorizontalSpacing];
            }
            
            _wrapperView.frame = CGRectMake(0, 0, leftOffset - [CameraModeControl _buttonHorizontalSpacing], 20);
        ...
        else
        {
            ...
            for (UIButton *button in _buttons)
            {
                ...
                topOffset += button.frame.size.height + CameraModeControlVerticalInteritemSpace;
            }
            
            _wrapperView.frame = CGRectMake(33, 0, self.frame.size.width, topOffset - CameraModeControlVerticalInteritemSpace);
        }
...
- (UIButton *)_createButtonForMode:(PGCameraMode)mode title:(NSString *)title
{
    ...
    button.tag = mode;
    button.titleLabel.font = [CameraInterfaceAssets normalFontOfSize:13];
    [button setAttributedTitle:[[NSAttributedString alloc] initWithString:title attributes:@{ NSForegroundColorAttributeName: [CameraInterfaceAssets normalColor], NSKernAttributeName: @(_kerning) }] forState:UIControlStateNormal];
    [button setAttributedTitle:[[NSAttributedString alloc] initWithString:title attributes:@{ NSForegroundColorAttributeName: [CameraInterfaceAssets accentColor], NSKernAttributeName: @(_kerning) }] forState:UIControlStateSelected];
