...
@interface CameraVolumeButtonHandler ()
...
@implementation CameraVolumeButtonHandler
...
static void PGButtonHandlerEnableMonitoring(bool enable)
{
    ...
    if (methodImpl != NULL)
        methodImpl([UIApplication sharedApplication], methodSelector, enable);
}
...
- (void)enableIn:(NSTimeInterval)timeInterval
{
    ...
    DispatchAfter(timeInterval, dispatch_get_main_queue(), ^
    {
