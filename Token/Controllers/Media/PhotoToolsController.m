...
@interface PhotoToolsController () <PhotoEditorCollectionViewToolsDataSource>
{
    NSArray *_tools;
    ...
    PhotoEditorCollectionView *_portraitCollectionView;
    PhotoEditorCollectionView *_landscapeCollectionView;
...
@property (nonatomic, weak) PhotoEditorItemController *editorItemController;

@end

@implementation PhotoToolsController
...
- (instancetype)initWithPhotoEditor:(PhotoEditor *)photoEditor previewView:(PhotoEditorPreviewView *)previewView
{
    ...
    if (self != nil)
    {
        ...
        for (PhotoTool *tool in photoEditor.tools)
        {
            if (!tool.isHidden)
...
- (void)loadView
{
    ...
    __weak PhotoToolsController *weakSelf = self;
    _interactionEnded = ^
    {
        __strong PhotoToolsController *strongSelf = weakSelf;
        if (strongSelf == nil)
            return;
    ...
    for (PhotoTool *tool in _tools)
    {
        NSString *title = tool.title;
        CGFloat width = 0.0f;
        if ([title respondsToSelector:@selector(sizeWithAttributes:)])
            width = CGCeil([title sizeWithAttributes:@{ NSFontAttributeName:[PhotoEditorInterfaceAssets editorItemTitleFont] }].width);
        else
            width = CGCeil([title sizeWithFont:[PhotoEditorInterfaceAssets editorItemTitleFont]].width);
    ...
    _portraitCollectionView = [[PhotoEditorCollectionView alloc] initWithOrientation:UIInterfaceOrientationPortrait cellWidth:maxTitleWidth];
    ...
    _portraitCollectionView.frame = CGRectMake(0, 0, collectionViewSize, PhotoEditorPanelSize);
    ...
    _landscapeCollectionView = [[PhotoEditorCollectionView alloc] initWithOrientation:UIInterfaceOrientationLandscapeLeft cellWidth:maxTitleWidth];
    ...
    _landscapeCollectionView.frame = CGRectMake(0, 0, PhotoEditorPanelSize, collectionViewSize);
...
{
    PhotoEditorItemController *controller = self.editorItemController;
    if (controller != nil)
...
- (NSInteger)numberOfToolsInCollectionView:(PhotoEditorCollectionView *)__unused collectionView
{
    return _tools.count;
}

- (PhotoTool *)collectionView:(PhotoEditorCollectionView *)__unused collectionView toolAtIndex:(NSInteger)index
{
    return _tools[index];
}

- (void)collectionView:(PhotoEditorCollectionView *)__unused collectionView didSelectToolWithIndex:(NSInteger)index
{
    if (self.editorItemController != nil)
        return;
    
    PhotoTool *selectedTool = _tools[index];
    
    __weak PhotoToolsController *weakSelf = self;
    PhotoEditorItemController *controller = [[PhotoEditorItemController alloc] initWithEditorItem:selectedTool photoEditor:_photoEditor previewView:_previewView];
    ...
    controller.beginTransitionOut = ^
    {
        ...
        PhotoEditorPreviewView *previewView = strongSelf.previewView;
    ...
    switch (orientation)
    {
        case UIInterfaceOrientationLandscapeLeft:
        {
            [UIView performWithoutAnimation:^
            {
                ...
                _landscapeCollectionView.frame = CGRectMake(panelToolbarLandscapeSize - PhotoEditorPanelSize, 0, PhotoEditorPanelSize, _landscapeCollectionView.frame.size.height);
            }];
            ...
            _portraitCollectionView.frame = CGRectMake(0, 0, _portraitToolsWrapperView.frame.size.width, PhotoEditorPanelSize);
        }
            break;
        ...
        case UIInterfaceOrientationLandscapeRight:
        {
            [UIView performWithoutAnimation:^
            {
                ...
                _landscapeCollectionView.frame = CGRectMake(0, 0, PhotoEditorPanelSize, _landscapeCollectionView.frame.size.height);
            }];
        ...
        default:
        {
            ...
            _landscapeCollectionView.frame = CGRectMake(_landscapeCollectionView.frame.origin.x, _landscapeCollectionView.frame.origin.y, PhotoEditorPanelSize, _landscapeToolsWrapperView.frame.size.height);
