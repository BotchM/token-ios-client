...
@implementation CheckButtonView

static CGAffineTransform CheckButtonDefaultTransform;

- (instancetype)initWithStyle:(CheckButtonStyle)style
{
    CGSize size = CGSizeMake(32.0f, 32.0f);
    if (style == CheckButtonStyleGallery)
    ...
    if (self != nil)
    {
        ...
        {
            CheckButtonDefaultTransform = CGAffineTransformMakeRotation(-M_PI_4);
        ...
        if (backgroundImage == nil)
        {
            switch (style)
            {
                ...
                case CheckButtonStyleBar:
                {
                    ...
                    CGContextSetStrokeColorWithColor(context, ColorWithHex(0xcacacf).CGColor);
                    CGContextStrokeEllipseInRect(context, CGRectInset(rect, insideInset + 0.5f, insideInset + 0.5f));
        ...
        if (fillImage == nil)
        {
            switch (style)
            {
                ...
                default:
                {
                    ...
                    UIGraphicsBeginImageContextWithOptions(rect.size, false, 0);
                    CGContextRef context = UIGraphicsGetCurrentContext();
                    CGContextSetFillColorWithColor(context, ColorWithHex(0x29c519).CGColor);
...
- (void)_createCheckButtonDetailsIfNeeded
{
    ...
    _checkView.transform = CheckButtonDefaultTransform;
    ...
    if (_style == CheckButtonStyleGallery)
    {
