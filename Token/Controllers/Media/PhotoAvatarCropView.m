...
const CGFloat PhotoAvatarCropViewOverscreenSize = 1000;

@interface PhotoAvatarCropView () <UIScrollViewDelegate>
{
...
@implementation PhotoAvatarCropView
...
- (instancetype)initWithOriginalSize:(CGSize)originalSize screenSize:(CGSize)screenSize
{
    ...
    if (self != nil)
    {
        ...
        _topOverlayView.backgroundColor = [PhotoEditorInterfaceAssets cropTransparentOverlayColor];
        ...
        _leftOverlayView.backgroundColor = [PhotoEditorInterfaceAssets cropTransparentOverlayColor];
        ...
        _rightOverlayView.backgroundColor = [PhotoEditorInterfaceAssets cropTransparentOverlayColor];
        ...
        _bottomOverlayView.backgroundColor = [PhotoEditorInterfaceAssets cropTransparentOverlayColor];
...
- (void)updateCircleImageWithReferenceSize:(CGSize)referenceSize
{
    ...
    CGContextSetFillColorWithColor(context, [PhotoEditorInterfaceAssets cropTransparentOverlayColor].CGColor);
...
- (void)resetAnimated:(bool)animated
{
    ...
    if (animated)
    {
        ...
        [PhotoEditorAnimation performBlock:^(bool allFinished)
        {
            if (allFinished)
                _isAnimating = false;
...
{
    return PhotoEditorCrop(_imageView.image, nil, self.cropOrientation, 0.0f, self.cropRect, false, maxSize, _originalSize, true);
}
...
- (void)animateTransitionIn
{
    ...
    _scrollView.backgroundColor = [PhotoEditorInterfaceAssets toolbarBackgroundColor];
...
{
    CGRect topOverlayFrame = CGRectMake(0, -PhotoAvatarCropViewOverscreenSize, self.frame.size.width, PhotoAvatarCropViewOverscreenSize);
    CGRect leftOverlayFrame = CGRectMake(-PhotoAvatarCropViewOverscreenSize, -PhotoAvatarCropViewOverscreenSize, PhotoAvatarCropViewOverscreenSize, self.frame.size.height + 2 * PhotoAvatarCropViewOverscreenSize);
    CGRect rightOverlayFrame = CGRectMake(self.frame.size.width, -PhotoAvatarCropViewOverscreenSize, PhotoAvatarCropViewOverscreenSize, self.frame.size.height + 2 * PhotoAvatarCropViewOverscreenSize);
    CGRect bottomOverlayFrame = CGRectMake(0, self.frame.size.height, self.frame.size.width, PhotoAvatarCropViewOverscreenSize);
...
- (void)layoutSubviews
{
    ...
    if (_scrollView.superview == nil)
    {
        ...
        if (_snapshotView != nil)
        {
            ...
            {
                CGSize snapshotSize = ScaleToFillSize(_snapshotSize, snapshotFrame.size);
