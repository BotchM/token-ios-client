...
@interface MediaAvatarEditorTransition ()
{
    __weak PhotoEditorController *_controller;
    UIView *_curtainView;
    
    UIImageView *_fromTransitionView;
    ImageView *_toTransitionView;
}
@end

@implementation MediaAvatarEditorTransition
...
- (instancetype)initWithController:(PhotoEditorController *)controller fromView:(UIView *)fromView
{
    ...
    if (self != nil)
    {
        ...
        {
            id<ModernGalleryTransitionView> transitionView = (id<ModernGalleryTransitionView>)fromView;
...
- (void)presentAnimated:(bool)__unused animated
{
    ...
    _curtainView.backgroundColor = [UIColor redColor];
    _curtainView.alpha = 1.0;
    _curtainView.tag = 7;
    ...
    CGRect containerFrame = [PhotoEditorTabController photoContainerFrameForParentViewFrame:CGRectMake(0, 0, referenceViewSize.width, referenceViewSize.height) toolbarLandscapeSize:_controller.toolbarLandscapeSize orientation:orientation panelSize:0.0f];
    ...
    CGSize fittedSize = ScaleToFill(referenceImageSize, CGSizeMake(diameter, diameter));
    ...
    _toTransitionView = [[ImageView alloc] initWithFrame:fromTransitionFrame];
    ...
    CGSize toSize = ScaleToFill(referenceImageSize, referenceFrame.size);
