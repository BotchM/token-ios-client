                ...
                if (intent == MediaAssetsControllerIntentSendFile)
                {
                    ...
                    }] catch:^SSignal *(id error)
                    {
                        ...
                        return [inlineThumbnailSignal(asset) map:^id(UIImage *image)
                        {
                            ...
                            [[MediaAssetsController selectedItemmsss] addObject:image];
...
{
    if (_intent == MediaAssetsControllerIntentSendFile)
        return nil;
    
    if (iosMajorVersion() < 7)
    {
//        ModernBarButton *searchButton = [[ModernBarButton alloc] initWithImage:[UIImage imageNamed:@"NavigationSearchIcon.png"]];
...
{
//    __weak MediaAssetsController *weakSelf = self;
//    TGWebSearchController *searchController = [[TGWebSearchController alloc] initForAvatarSelection:(_intent == MediaAssetsControllerIntentSetProfilePhoto) embedded:true];
...
//    {
//        MediaAssetsController *strongSelf = weakSelf;
