...
#import "ModernGalleryImageItemContainerView.h"
...
@interface ModernGalleryController () <UIScrollViewDelegate, ModernGalleryScrollViewDelegate, ModernGalleryItemViewDelegate>//, TGKeyCommandResponder>
{
    ...
    ModernGalleryView *_view;
    UIView<ModernGalleryDefaultHeaderView> *_defaultHeaderView;
    UIView<ModernGalleryDefaultFooterView> *_defaultFooterView;
...
@implementation ModernGalleryController
...
- (void)setModel:(ModernGalleryModel *)model
{
    if (_model != model)
    {
        _model = model;
        
        __weak ModernGalleryController *weakSelf = self;
        _model.itemsUpdated = ^(id<ModernGalleryItem> item)
        {
            __strong ModernGalleryController *strongSelf = weakSelf;
            if (strongSelf != nil && !strongSelf->_isBeingDismissed)
                [strongSelf reloadDataAtItem:item synchronously:false];
        };
        
        _model.focusOnItem = ^(id<ModernGalleryItem> item)
        {
            __strong ModernGalleryController *strongSelf = weakSelf;
...
- (void)itemViewIsReadyForScheduledDismiss:(ModernGalleryItemView *)__unused itemView
{
    [self dismiss];
}

- (void)itemViewDidRequestInterfaceShowHide:(ModernGalleryItemView *)__unused itemView
{
    [_view showHideInterface];
}

- (void)itemViewDidRequestGalleryDismissal:(ModernGalleryItemView *)__unused itemView animated:(bool)animated
{
    if (_completedTransitionOut)
        _completedTransitionOut();
...
- (UIView *)itemViewDidRequestInterfaceView:(ModernGalleryItemView *)__unused itemView
{
...
- (ModernGalleryItemView *)dequeueViewForItem:(id<ModernGalleryItem>)item
{
    ...
    if (views.count == 0)
    {
        ...
        ModernGalleryItemView *itemView = [[itemClass alloc] init];
    ...
    ModernGalleryItemView *itemView = [views lastObject];
...
- (void)enqueueView:(ModernGalleryItemView *)itemView
{
    if (itemView == nil)
        return;
...
{
    for (ModernGalleryItemView *itemView in self->_visibleItemViews)
    {
...
{
    [super loadView];
    object_setClass(self.view, [ModernGalleryContainerView class]);
    
    self.view.frame = (CGRect){self.view.frame.origin, [UIScreen mainScreen].bounds.size};
    
    _reusableItemViewsByIdentifier = [[NSMutableDictionary alloc] init];
    _visibleItemViews = [[NSMutableArray alloc] init];
    
    UIView<ModernGalleryInterfaceView> *interfaceView = [_model createInterfaceView];
    if (interfaceView == nil)
        interfaceView = [[ModernGalleryDefaultInterfaceView alloc] initWithFrame:CGRectZero];
    ...
    _view.transitionOut = ^bool (CGFloat velocity)
    {
        ...
        if (strongSelf != nil)
        {
            ...
            id<ModernGalleryItem> focusItem = nil;
            if ([strongSelf currentItemIndex] < strongSelf.model.items.count)
                focusItem = strongSelf.model.items[[strongSelf currentItemIndex]];
            
            ModernGalleryItemView *currentItemView = nil;
            for (ModernGalleryItemView *itemView in strongSelf->_visibleItemViews)
            {
    ...
    if (_animateTransition) {
        ...
        ModernGalleryItemView *transitionFromItemView = nil;
        CGRect transitionInToViewContentRect = CGRectMake(0.0f, 0.0f, 1.0f, 1.0f);
        if (_beginTransitionIn && _model.focusItem != nil)
        {
            ModernGalleryItemView *itemView = nil;
            for (ModernGalleryItemView *visibleItemView in self->_visibleItemViews)
            {
        ...
        {
            for (ModernGalleryItemView *itemView in _visibleItemViews)
            {
...
- (void)animateView:(UIView *)view frameFrom:(CGRect)fromFrame to:(CGRect)toFrame velocity:(CGPoint)__unused velocity rotationFrom:(CGFloat)fromRotation to:(CGFloat)toRotation animatingIn:(bool)animatingIn completion:(void (^)(bool))completion
{
    ...
    AnimationBlockDelegate *delegate = [[AnimationBlockDelegate alloc] initWithLayer:view.layer];
...
- (void)animateTransitionInFromView:(UIView *)fromView toView:(UIView *)toView toViewContentRect:(CGRect)toViewContentRect
{
    ...
    if ([fromView conformsToProtocol:@protocol(ModernGalleryTransitionView)] && [fromView respondsToSelector:@selector(transitionContentRect)])
    {
        CGRect fromContentRect = [(id<ModernGalleryTransitionView>)fromView transitionContentRect];
    ...
    if ([fromView conformsToProtocol:@protocol(ModernGalleryTransitionView)])
    {
        UIImage *transitionImage = [(id<ModernGalleryTransitionView>)fromView transitionImage];
    ...
    [self animateView:toView frameFrom:fromFrame to:toView.frame velocity:CGPointZero rotationFrom:fromRotationZ to:0.0f animatingIn:true completion:^(bool __unused finished)
    {
        ...
        if (strongSelf != nil)
        {
            ...
            {
                for (ModernGalleryItemView *visibleItemView in strongSelf->_visibleItemViews)
                {
...
- (void)animateTransitionOutFromView:(UIView *)fromView fromViewContentRect:(CGRect)fromViewContentRect toView:(UIView *)toView velocity:(CGPoint)velocity
{
    ...
    ModernGalleryComplexTransitionDescription *transitionDesc = nil;
    
    TG_TIMESTAMP_MEASURE(out)
    
    if ([toView conformsToProtocol:@protocol(ModernGalleryTransitionView)])
    {
        if ([toView respondsToSelector:@selector(hasComplexTransition)])
        {
            if ([(id<ModernGalleryTransitionView>)toView hasComplexTransition])
                transitionDesc = [(id<ModernGalleryTransitionView>)toView complexTransitionDescription];
        }
        else
        {
            UIImage *transitionImage = [(id<ModernGalleryTransitionView>)toView transitionImage];
            if (transitionImage != nil)
            {
    ...
    else
    {
        ...
        {
            if ([fromView isKindOfClass:[ModernGalleryImageItemContainerView class]])
            {
                UIView *contentView = ((ModernGalleryImageItemContainerView *)fromView).contentView();
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    if (_previewMode && animated)
    {
        ...
        {
            id<ModernGalleryItem> item = [self currentItem];
...
{
    [self setCurrentItemIndex:index direction:ModernGalleryScrollAnimationDirectionDefault animated:animated];
}
...
- (void)setCurrentItemIndex:(NSUInteger)index direction:(ModernGalleryScrollAnimationDirection)direction animated:(bool)animated
{
    ...
    if (animated)
    {
        ...
        if (direction == ModernGalleryScrollAnimationDirectionDefault)
            direction = (index > [self currentItemIndex]) ? ModernGalleryScrollAnimationDirectionRight : ModernGalleryScrollAnimationDirectionLeft;
        ...
        switch (direction)
        {
            case ModernGalleryScrollAnimationDirectionLeft:
                incomingItemStartPosition.x += -referenceSize.width - 2 * ModernGalleryItemPadding;
                outgoingItemTargetPosition.x = referenceSize.width * 1.5f + 2 * ModernGalleryItemPadding;
                break;
                
            case ModernGalleryScrollAnimationDirectionRight:
                incomingItemStartPosition.x += referenceSize.width + 2 * ModernGalleryItemPadding;
                outgoingItemTargetPosition.x = -(referenceSize.width / 2) - 2 * ModernGalleryItemPadding;
                break;
...
- (void)reloadDataAtItem:(id<ModernGalleryItem>)atItem synchronously:(bool)synchronously
{
    NSMutableIndexSet *removeIndices = nil;
    
    id<ModernGalleryItem> focusItem = atItem;
    ...
    for (ModernGalleryItemView *itemView in _visibleItemViews)
    {
        ...
        for (id<ModernGalleryItem> item in _model.items)
        {
            itemIndex++;
...
- (void)scrollViewBoundsChanged:(CGRect)bounds synchronously:(bool)synchronously
{
    ...
    if (leftmostVisibleItemIndex <= rightmostVisibleItemIndex && _model.items.count != 0)
    {
        ...
        {
            ModernGalleryItemView *itemView = _visibleItemViews[i];
            ...
            else
            {
                ...
                CGRect itemFrame = CGRectMake(itemWidth * itemView.index + ModernGalleryItemPadding, 0.0f, itemWidth - ModernGalleryItemPadding * 2.0f, bounds.size.height);
        ...
        for (NSUInteger i = leftmostVisibleItemIndex; i <= rightmostVisibleItemIndex; i++)
        {
            ...
            {
                id<ModernGalleryItem> item = _model.items[i];
                ModernGalleryItemView *itemView = [self dequeueViewForItem:item];
                if (itemView != nil)
                {
                    itemView.frame = CGRectMake(itemWidth * i + ModernGalleryItemPadding, 0.0f, itemWidth - ModernGalleryItemPadding * 2.0f, bounds.size.height);
...
//            if (![itemHeaderView isKindOfClass:[ModernGalleryEmbeddedStickersHeaderView class]]) {
//                titleAlpha -= alpha;
//            }
        }
