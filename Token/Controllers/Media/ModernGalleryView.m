...
@interface ModernGalleryView () <UIGestureRecognizerDelegate>
{
...
@implementation ModernGalleryView
...
- (instancetype)initWithFrame:(CGRect)frame itemPadding:(CGFloat)itemPadding interfaceView:(UIView<ModernGalleryInterfaceView> *)interfaceView previewMode:(bool)previewMode previewSize:(CGSize)previewSize
{
    ...
    if (self != nil)
    {
        ...
        _scrollView = [[ModernGalleryScrollView alloc] initWithFrame:CGRectMake(-_itemPadding, 0.0f, frame.size.width + itemPadding * 2.0f, frame.size.height)];
        [_scrollViewContainer addSubview:_scrollView];
        
        _interfaceView = interfaceView;
        _interfaceView.frame = bounds;
        _interfaceView.hidden = _previewMode;
        __weak ModernGalleryView *weakSelf = self;
        ...
        if (hasSwipeGesture)
        {
            ModernGalleryZoomableScrollViewSwipeGestureRecognizer *swipeRecognizer = [[ModernGalleryZoomableScrollViewSwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipeGesture:)];
...
- (void)swipeGesture:(ModernGalleryZoomableScrollViewSwipeGestureRecognizer *)recognizer
{
