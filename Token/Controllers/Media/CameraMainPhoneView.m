...
#import "CameraFlashActiveView.h"
...
@interface CameraTopPanelView : UIView

@property (nonatomic, copy) bool(^isPointInside)(CGPoint point);

@end

@implementation CameraTopPanelView
...
@interface CameraMainPhoneView () <ASWatcher>
{
    CameraTopPanelView *_topPanelView;
    ...
    CameraFlashActiveView *_flashActiveView;
...
@implementation CameraMainPhoneView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        __weak CameraMainPhoneView *weakSelf = self;
        
        _topPanelView = [[CameraTopPanelView alloc] init];
        _topPanelView.backgroundColor = [CameraInterfaceAssets transparentPanelBackgroundColor];
        _topPanelView.isPointInside = ^bool(CGPoint point)
        {
            __strong CameraMainPhoneView *strongSelf = weakSelf;
            if (strongSelf == nil)
                return false;
        ...
        _bottomPanelBackgroundView.backgroundColor = [CameraInterfaceAssets transparentPanelBackgroundColor];
        ...
        _doneButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 60, 44)];
        ...
        [_doneButton setTitle:TGLocalized(@"Done") forState:UIControlStateNormal];
        [_doneButton setTintColor:[CameraInterfaceAssets normalColor]];
        ...
        _shutterButton = [[CameraShutterButton alloc] initWithFrame:CGRectMake((frame.size.width - 66) / 2, 10, 66, 66)];
        ...
        _modeControl = [[CameraModeControl alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, _modeControlHeight)];
        [_bottomPanelView addSubview:_modeControl];
        
        _flipButton = [[CameraFlipButton alloc] initWithFrame:CGRectMake(0, 0, 56, 56)];
        ...
        _flashControl = [[CameraFlashControl alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, CameraFlashControlHeight)];
        [_topPanelView addSubview:_flashControl];
        
        _timecodeView = [[CameraTimeCodeView alloc] initWithFrame:CGRectMake((frame.size.width - 120) / 2, 12, 120, 20)];
        ...
        _videoLandscapePanelView.backgroundColor = [CameraInterfaceAssets transparentPanelBackgroundColor];
        ...
        _flashActiveView = [[CameraFlashActiveView alloc] initWithFrame:CGRectMake((frame.size.width - 40) / 2, frame.size.height - _bottomPanelHeight - 37, 40, 21)];
        [self addSubview:_flashActiveView];
        
        _zoomView = [[CameraZoomView alloc] initWithFrame:CGRectMake(10, frame.size.height - _bottomPanelHeight - 18, frame.size.width - 20, 1.5f)];
        ...
        _segmentsView = [[CameraSegmentsView alloc] init];
...
- (void)setupTooltip
{
    ...
    [actions addObject:[[NSDictionary alloc] initWithObjectsAndKeys:TGLocalized(@"TapAndHoldForVideo"), @"title", nil]];
...
- (void)_layoutTopPanelSubviewsForInterfaceOrientation:(UIInterfaceOrientation)orientation
{
    ...
    if (UIInterfaceOrientationIsLandscape(orientation) && _flashControl.interfaceOrientation == orientation && _flashControl.superview == _topPanelView)
    {
        if (orientation == UIInterfaceOrientationLandscapeLeft)
            _flashControl.frame = CGRectMake(7, 0, CameraFlashControlHeight, 370);
        else if (orientation == UIInterfaceOrientationLandscapeRight)
            _flashControl.frame = CGRectMake(7, 0, CameraFlashControlHeight, 370);
    }
    else
    {
        _flashControl.frame = CGRectMake(0, (superviewSize.height - CameraFlashControlHeight) / 2, superviewSize.width, CameraFlashControlHeight);
    }
