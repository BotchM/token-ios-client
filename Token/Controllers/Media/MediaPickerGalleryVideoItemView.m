...
//#import "MediaPickerGalleryVideoScrubber.h"
...
@interface MediaPickerGalleryVideoItemView()// <MediaPickerGalleryVideoScrubberDataSource, MediaPickerGalleryVideoScrubberDelegate>
{
    ...
    MenuContainerView *_tooltipContainerView;
   ...
   // MediaPickerGalleryVideoScrubber *_scrubberView;
...
@property (nonatomic, strong) MediaPickerGalleryVideoItem *item;

@end

@implementation MediaPickerGalleryVideoItemView
...
//        
//        _actionButton = [[ModernButton alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
...
//        
//        _progressView = [[MessageImageViewOverlayView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
...
//        _scrubberPanelView.backgroundColor = [PhotoEditorInterfaceAssets toolbarTransparentBackgroundColor];
//        _scrubberPanelView.hidden = true;
//        [_headerView addSubview:_scrubberPanelView];
//        
//        _scrubberView = [[MediaPickerGalleryVideoScrubber alloc] initWithFrame:_scrubberPanelView.bounds];
...
//    
//    __weak MediaPickerGalleryVideoItemView *weakSelf = self;
//    [_downloadDisposable setDisposable:[[[MediaAssetImageSignals avAssetForVideoAsset:self.item.asset allowNetworkAccess:true] deliverOn:[SQueue mainQueue]] startWithNext:^(id next)
//    {
//        __strong MediaPickerGalleryVideoItemView *strongSelf = weakSelf;
//        if (strongSelf == nil)
//            return;
//        
...
//}
//
//- (void)setItem:(MediaPickerGalleryVideoItem *)item synchronously:(bool)synchronously
//{
...
//                VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[strongSelf.item.editingContext adjustmentsForItem:strongSelf.item.editableMediaItem];
...
//        NSString *dimensions = [MediaPickerGalleryVideoItemView _stringForDimensions:next.dimensions];
//        
...
//            return;
//        
//        MediaPickerGalleryVideoItem *item = strongSelf.item;
//        NSTimeInterval videoDuration = next.doubleValue;
//        strongSelf->_videoDuration = videoDuration;;
//        
//        strongSelf->_scrubberView.allowsTrimming = (!item.asFile && ((item.asset != nil && !(item.asset.subtypes & MediaAssetSubtypeVideoHighFrameRate)) || item.avAsset != nil) && videoDuration >= VideoEditMinimumTrimmableDuration);
//        
//        VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[item.editingContext adjustmentsForItem:item.editableMediaItem];
...
//    
//    VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[self.item.editingContext adjustmentsForItem:self.item.editableMediaItem];
...
//    }
//    
//    _scrubberView.maximumLength = adjustments.sendAsGif ? VideoEditMaximumGifDuration : 0.0;
//    
...
//        return;
//
//    CGSize fittedSize = ScaleToSize(videoFrameSize, self.frame.size);
...
//        }
//        
//        CGContextConcatCTM(UIGraphicsGetCurrentContext(), VideoCropTransformForOrientation(cropOrientation, _playerWrapperView.bounds.size, false));
//
...
//        itemSignal = [MediaAssetImageSignals playerItemForVideoAsset:self.item.asset];
...
//        
//        strongSelf->_didPlayToEndObserver = [[ObserverProxy alloc] initWithTarget:strongSelf targetSelector:@selector(playerItemDidPlayToEndTime:) name:AVPlayerItemDidPlayToEndTimeNotification object:playerItem];
//        
//        strongSelf->_videoView = [[ModernGalleryVideoView alloc] initWithFrame:strongSelf->_playerView.bounds player:strongSelf->_player];
...
//
//- (NSTimeInterval)videoScrubberDuration:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
//    return _videoDuration;
//}
//
//- (CGFloat)videoScrubberThumbnailAspectRatio:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
...
//}
//
//- (void)videoScrubberDidBeginScrubbing:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
//    if (_player == nil)
...
//}
//
//- (void)videoScrubberDidEndScrubbing:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
//    if (_wasPlayingBeforeScrubbing)
//        [self play];
//}
//
//- (void)videoScrubber:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber valueDidChange:(NSTimeInterval)position
//{
...
//}
//
//- (void)videoScrubberDidBeginEditing:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
//    if (_player == nil)
//        [self preparePlayerAndPlay:false];
//    
...
//}
//
//- (void)videoScrubberDidEndEditing:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
...
//}
//
//- (void)videoScrubber:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber editingStartValueDidChange:(NSTimeInterval)startValue
//{
//    [self _seekToPosition:startValue manual:true];
//}
//
//- (void)videoScrubber:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber editingEndValueDidChange:(NSTimeInterval)endValue
//{
...
//    }
//    
//    VideoEditAdjustments *updatedAdjustments = [VideoEditAdjustments editAdjustmentsWithOriginalSize:_videoDimensions cropRect:cropRect cropOrientation:adjustments.cropOrientation cropLockedAspectRatio:adjustments.cropLockedAspectRatio cropMirrored:adjustments.cropMirrored trimStartValue:trimStartValue trimEndValue:trimEndValue paintingData:adjustments.paintingData sendAsGif:sendAsGif preset:adjustments.preset];
...
//            
//            _tooltipContainerView = [[MenuContainerView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, parentView.frame.size.width, parentView.frame.size.height)];
//            [parentView addSubview:_tooltipContainerView];
//            
//            NSMutableArray *actions = [[NSMutableArray alloc] init];
//            [actions addObject:[[NSDictionary alloc] initWithObjectsAndKeys:TGLocalized(@"VideoMuteDescription"), @"title", nil]];
...
//                
//                CGSize fillSize = ScaleToFillSize(_videoDimensions, thumbnailSize);
//                
...
//        
//        VideoEditAdjustments *updatedAdjustments = [VideoEditAdjustments editAdjustmentsWithOriginalSize:_videoDimensions cropRect:cropRect cropOrientation:cropOrientation cropLockedAspectRatio:cropLockedAspectRatio cropMirrored:adjustments.cropMirrored trimStartValue:_scrubberView.trimStartValue trimEndValue:_scrubberView.trimEndValue paintingData:adjustments.paintingData sendAsGif:adjustments.sendAsGif preset:adjustments.preset];
//        
//        [self.item.editingContext setAdjustments:updatedAdjustments forItem:self.item.editableMediaItem];
//    }
//}
...
//}
//
//- (void)videoScrubber:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber requestThumbnailImagesForTimestamps:(NSArray *)timestamps size:(CGSize)size isSummaryThumbnails:(bool)isSummaryThumbnails
//{
...
//    if (self.item.asset != nil)
//        thumbnailsSignal = [MediaAssetImageSignals videoThumbnailsForAsset:self.item.asset size:size timestamps:timestamps];
//    else if (self.item.avAsset != nil)
//        thumbnailsSignal = [MediaAssetImageSignals videoThumbnailsForAVAsset:self.item.avAsset size:size timestamps:timestamps];
//
...
//}
//
//- (void)videoScrubberDidFinishRequestingThumbnails:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
...
//}
//
//- (void)videoScrubberDidCancelRequestingThumbnails:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber
//{
//    _requestingThumbnails = false;
//}
//
//- (CGSize)videoScrubberOriginalSize:(MediaPickerGalleryVideoScrubber *)__unused videoScrubber cropRect:(CGRect *)cropRect cropOrientation:(UIImageOrientation *)cropOrientation cropMirrored:(bool *)cropMirrored
//{
//    VideoEditAdjustments *adjustments = (VideoEditAdjustments *)[self.item.editingContext adjustmentsForItem:self.item.editableMediaItem];
//    if (cropRect != NULL)
